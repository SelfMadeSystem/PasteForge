...
public class OutlinedCylinderESP extends ESPModule {
    ...
    public void esp(Entity e) {
        ...
        for (int i = 0; i < 360; i += add) {
            ...
            bufferBuilder.pos(center.x + MathUtil.cos(i) * sizeX, aabb.minY, center.z + MathUtil.sin(i) * sizeZ).endVertex();
            bufferBuilder.pos(center.x + MathUtil.cos(i) * sizeX, aabb.maxY, center.z + MathUtil.sin(i) * sizeZ).endVertex();

            bufferBuilder.pos(center.x + MathUtil.cos(i) * sizeX, aabb.maxY, center.z + MathUtil.sin(i) * sizeZ).endVertex();
            bufferBuilder.pos(center.x + MathUtil.cos(i + add) * sizeX, aabb.maxY, center.z + MathUtil.sin(i + add) * sizeZ).endVertex();
        }
